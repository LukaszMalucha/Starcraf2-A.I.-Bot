...
class Apollyon(sc2.BotAI):
    def __init__(self, use_model=False):
        ...
        if self.use_model:
            print("using model")
            self.model = keras.models.load_model("Apollyon_Terran")
            ...
            if self.units(FACTORY).amount < 3:
                if self.units(MARINE).amount > 2 and self.units(MARAUDER).amount > 2:
                    if self.can_afford(FACTORY):    
                        await self.build(FACTORY, near=centers.position.towards(self.game_info.map_center, 8))
        ...
        if self.supply_used < 190:           
            if self.units(BARRACKS).ready.noqueue and self.units(FACTORY).ready.noqueue:
                if self.use_model:
                    ...
                    choice_dict ={0: "Marauder",
                                  1: "Cyclone",
                                  2: "Thor",
                                  3: "Medivac"}

                    print("I'm going to build {}".format(choice_dict[choice]))        
                else:    
                    ...
                    if choice == 1:                
                        for factory in self.units(FACTORY).ready.noqueue:
                            if self.can_afford(CYCLONE) and self.supply_left > 0:
                                if self.units(CYCLONE).amount < 20 and self.can_afford(CYCLONE):
